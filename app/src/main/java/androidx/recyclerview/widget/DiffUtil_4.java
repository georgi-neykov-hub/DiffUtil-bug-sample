...
public class DiffUtil_4 {
    ...
    public static class DiffResult {
        ...
        private static void printPositionMappings(String label, int[] statuses, int size) {
            System.out.println(label + "\n");
            for (int i = 0; i < size; i++) {
                int positionAndStatus = statuses[i];
                int position = positionAndStatus >> FLAG_OFFSET;
                int status = positionAndStatus & FLAG_MASK;
                String stateString;
                switch (status) {
                    case 0:
                        stateString = "(No match)";
                        break;
                    case FLAG_CHANGED:
                        stateString = " (Change)";
                        break;
                    case FLAG_NOT_CHANGED:
                        stateString = " (No Change)";
                    case FLAG_MOVED_CHANGED:
                        stateString = "(Move + Change)";
                        break;
                ...
                System.out.println(i + " -> " + (status == 0 ? "-" : String.valueOf(position)) + " " + stateString);
            }
            System.out.println("");
        }
        ...
        DiffResult(Callback callback, List<Snake> snakes, int[] oldItemStatuses,
            ...
            findMatchingItems();
            if (mOldListSize > 0)
                printPositionMappings("Old -> New", mOldItemStatuses, mOldListSize);
            if (mNewListSize > 0)
                printPositionMappings("New -> Old", mNewItemStatuses, mNewListSize);
        }
            ...
            for (int i = mSnakes.size() - 1; i >= 0; i--) {
                ...
                if (mDetectMoves) {
                    while (posOld > endX) {
                        ...
                        boolean match = findAddition(posOld, posNew, i);
                        posOld--;
                    }
                    while (posNew > endY) {
                        // this is an addition. Check remaining snakes to see if this was removed
                        // before
                        boolean match = findRemoval(posOld, posNew, i);
                        posNew--;
                    }
                }
                for (int j = 0; j < snake.size; j++) {
                    ...
                    final boolean contentsTheSame = mCallback
                            .areContentsTheSame(oldItemPos, newItemPos);
                    final int changeFlag = contentsTheSame ? FLAG_NOT_CHANGED : FLAG_CHANGED;
        ...
        private boolean findAddition(int x, int y, int snakeIndex) {
            if (mOldItemStatuses[x - 1] != 0) {
                return true; // already set by a latter item
            }
            return findMatchingItem(x, y, snakeIndex, false);
        }

        private boolean findRemoval(int x, int y, int snakeIndex) {
            if (mNewItemStatuses[y - 1] != 0) {
                return true; // already set by a latter item
            }
            return findMatchingItem(x, y, snakeIndex, true);
        }
            ...
            for (int i = snakeIndex; i >= 0; i--) {
                ...
                if (removal) {
                    ...
                    for (int pos = curX - 1; pos >= endX; pos--) {
                        if (mCallback.areItemsTheSame(pos, myItemPos) && mOldItemStatuses[pos] == 0) {
                            // found!
                    ...
                    for (int pos = curY - 1; pos >= endY; pos--) {
                        if (mCallback.areItemsTheSame(myItemPos, pos) && mNewItemStatuses[pos] == 0) {
                            // found
